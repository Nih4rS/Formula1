...
st.caption("Analysis-grade dashboards over FastF1 data. Click map turns, compare drivers, and browse the full season schedule.")
...
with tab_schedule:
    ...
    # pick year for schedule separately (kept sticky)
    years_all = list_years() or list(range(2014, datetime.now().year + 1))
    ...
    if df_sched.empty:
        ...
        if ns is not None:
            with cols[0]:
                st.metric("Next", f"{ns['EventName']}", help=f"{ns['Session']}")
            with cols[1]:
                st.metric("When (Local)", ns['StartLocal'].strftime("%a %d %b %H:%M"))
            with cols[2]:
                st.metric("When (UTC)", ns['StartUTC'].strftime("%a %d %b %H:%M"))
            with cols[3]:
                st.metric("Session", ns["Session"])
            with cols[4]:
                # pseudo-live countdown (updates when user re-runs)
                st.metric("Countdown", countdown_str(ns["StartUTC"]))

        st.markdown("### Sessions")
        show_local = st.toggle("Show local time", value=True, help="Toggle between Local and UTC start times.")
        df_view = df_sched.copy()
        df_view["Start (Local)"] = df_view["StartLocal"].dt.strftime("%a %d %b %H:%M")
        df_view["Start (UTC)"] = pd.to_datetime(df_view["StartUTC"]).dt.strftime("%a %d %b %H:%M")
        df_view["Session"] = df_view["Session"]
        df_view = df_view[["EventName", "Session", "Start (Local)" if show_local else "Start (UTC)"]]

        st.dataframe(df_view, use_container_width=True, height=420)

        # Jump to Analysis controls
        st.divider()
        st.markdown("#### Jump to Analysis")
        # choose event & session from schedule context to sync with analysis tab
        evs = sorted(df_sched["EventName"].unique().tolist())
        choose_event = st.selectbox("Grand Prix", evs, index=evs.index(ns["EventName"]) if ns is not None else 0, key="sched_jump_event")

        # sessions available for that GP (order by planned timeline)
        ...
        if st.button("Go to Analysis"):
            # parse the chosen code back
            code = gp_sessions.iloc[[s.split("(")[-1].startswith for s in []]] if False else None  # dummy to appease linter
            # Extract code from the label like "Qualifying (Q)"
            lbl = st.session_state["sched_jump_session"]
            code = lbl.split("(")[-1].rstrip(")")
            # Sync primary app state and switch to Analysis tab on rerun
...
with tab_analysis:
    ...
    year = st.sidebar.selectbox("Year", years, index=years.index(st.session_state["year"]), key="year")
    ...
    if len(drivers) < 2:
        st.error("Need at least two drivers in data to compare.")
        st.stop()
    ...
    for drv in cmp_multi:
        try:
            ...
            dx, dy = lap_delta(
                ref["distance_m"], ref["cum_lap_time_s"],
    ...
    with col1:
        if delta_traces:
            fig = multi_line(
                "Lap Time Delta vs Distance (positive = slower than reference)",
    ...
    # ---------------------------
    # Throttle / Brake overlays (whole lap)
    # ---------------------------
    st.subheader("Throttle and Brake overlays")
    tb_pick = st.multiselect(
        "Choose drivers",
        [ref_driver] + cmp_choices,
        default=[ref_driver] + cmp_choices[:1],
        key="tb_drivers",
    )
